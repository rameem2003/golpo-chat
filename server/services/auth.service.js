...
const createNewUser = async ({ name, email, password, avatarLink }) => {
  try {
...
const createResetPasswordTokenLink = async (userID) => {
  try {
    ...
    let tokenLink = `${process.env.MAIN_SITE_URL}/reset-password?token=${token}`;
...
const createEmailLink = (email, token, req) => {
  // const url = new URL(
  //   process.env.SYSTEM_ENV == "development"
  ...
  // );
  const url = new URL(
    `${req.protocol}://${req.host}${process.env.API_VERSION_BASE}/auth/verify-email`,
  );
